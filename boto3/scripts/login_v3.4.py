            ...
            for Tag in instance['Tags']:
                if (Tag['Key'] == 'Name'):
                    if search in Tag['Value']:
                        InstanceList.append([idx, Tag.get('Value','NoName'), instance['InstanceId'], instance['PrivateIpAddress']])
                        idx = idx + 1
...
def login(file,InstanceList,Bastion,verbose):
    ...
    if verbose:
        command = 'ssh -i '+file+' -v ec2-user@'+choice_instance+' -o "proxycommand=ssh -W %h:%p -i '+file+' ec2-user@'+Bastion+'"'
    else:
        command = 'ssh -i '+file+' -qt ec2-user@'+choice_instance+' -o "proxycommand=ssh -W %h:%p -i '+file+' ec2-user@'+Bastion+'"'
    os.system(command)
